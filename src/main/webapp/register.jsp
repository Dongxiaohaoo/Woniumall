            ...
            <div class="box clearfix">
                <form action='user' method="post" onsubmit="return validateForm()">
                    ...
                    <table class="form_table f_l">
                        ...
                        <tr>
                            <th>用户名：</th>
                            <td><input class="gray" name='account' id="name"
                                       type="text" value="" tabindex="1"/><label for="name" id="namemsg">请填写用户名，格式为2-20个字符，可以为字数，数字下划线和中文</label>
                            </td>
                        </tr>
                        <tr>
                            <th>设置密码：</th>
                            <td><input class="gray" type="password"
                                       name='password' id="password" tabindex="2"/><label>填写登录密码，6-32个字符</label></td>
                        </tr>
                        <tr>
                            <th>确认密码：</th>
                        ...
                        <tr>
                            <th>手机号码：</th>
                            <td><input class="gray" name='phoneNum' id="phoneNum" type="text"
                                       tabindex="4"><label></label></td>
                        </tr>
                        <tr>
                            <th>电子邮箱：</th>
                            <td><input class="gray" name='email' id="email" type="text" tabindex="5"><label></label>
                            </td>
                        </tr>
                        <tr>
                            <th>验证码：</th>
                            <td><input type='text' class='gray_s' name='cap' alt='填写下面图片所示的字符'/>
                                <label>填写下面图片计算结果</label></td>
                        </tr>
                        <tr class="low">
                            <th></th>
                            <td>
                                ...
                                <span class="light_gray">看不清？<a class="link"
                                                                onclick="changeImg(document.getElementsByTagName('img')[0])">换一张</a></span>
                            </td>
                        </tr>
                        <tr>
                            <td></td>
                ...
                <div class="agreement gray">
                    ...
                    议，双方应尽力友好协商解决；协商不成时，任何一方均可向本站所在地的人民法院提起诉讼。
                </div>
            </div>
        </div>
    </div>
...
<script>
    function checkAccount() {
        ...
        let patt = /^[a-z0-9_-]{2,20}$/;
        let test = patt.test(account);
        if (!test) {
            document.getElementsByTagName("label")[0].innerText = "账号输入有误！";
            return false;
        }
        return true;
    }
    ...
    function checkPassword() {
        let patt = /^[a-zA-Z0-9_-]{6,32}$/;
        let password = document.getElementById("password").value;
        if (!patt.test(password)) {
            document.getElementsByTagName("label")[1].innerText = "密码格式错误！";
            return false;
        }
    ...
    function checkSame() {
        ...
        if (password != repassword) {
            document.getElementsByTagName("label")[2].innerText = "密码输入不一致！";
            return false;
        }
    ...
    function checkPhone() {
        ...
        let patt = /^1(3[0-9]|5[0-3,5-9]|7[1-3,5-8]|8[0-9])\d{8}$/;
        if (!patt.test(phoneNum)) {
            document.getElementsByTagName("label")[3].innerText = "手机号格式错误！";
            return false;
        }
    ...
    function checkEmail() {
        ...
        let patt = /^[a-z\d]+(\.[a-z\d]+)*@([\da-z](-[\da-z])?)+(\.{1,2}[a-z]+)+$/;
        if (!patt.test(email)) {
            document.getElementsByTagName("label")[4].innerText = "邮箱格式错误！";
            return false;
        }
        //异步请求校验邮箱
        axios.get("user?opr=AjaxUsernameIsExist&email=" + email).then(function (res) {
            document.getElementsByTagName("label")[4].innerHTML = "<span style='color: green'>邮箱可以使用</span>";
        }).catch(function (error) {
            document.getElementsByTagName("label")[4].innerHTML = "<span style='color: green'>邮箱已被注册...</span>";
        });
        return true;
    }
    ...
    document.getElementById("name").addEventListener("change", function () {
        ...
        let account = document.getElementById("name").value;
        axios.get("user?opr=AjaxUsernameIsExist&account=" + account).then(function (res) {
            document.getElementsByTagName("label")[0].innerHTML = "<span style='color: green'>账号可以使用</span>";
        }).catch(function (error) {
            document.getElementsByTagName("label")[0].innerHTML = "<span style='color: red'>账号已被使用</span>";
        });
    });
    ...
    document.getElementById("phoneNum").addEventListener("change", function () {
        checkPhone();
    })
    ...
    function validateForm() {
        if (checkAccount() && checkPassword() && checkSame() && checkPhone() && checkEmail()) {
            return true;
        }
        return false;
    }
